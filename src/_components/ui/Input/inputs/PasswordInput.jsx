...
const getStrength = (password) => {
    ...
    const levels = [
        { labelKey: 'very weak', color: 'bg-red-500' },
        { labelKey: 'weak', color: 'bg-orange-500' },
        { labelKey: 'fair', color: 'bg-yellow-500' },
        { labelKey: 'good', color: 'bg-blue-500' },
        { labelKey: 'strong', color: 'bg-green-500' },
        { labelKey: 'very strong', color: 'bg-emerald-500' },
    ];
...
export const PasswordInput = forwardRef(({
    ...
    const handleChange = (e) => {
        ...
        const syntheticEvent = {
            ...e,
            target: { ...e.target, name: props?.name || e.target.name || '', value: e.target.value },
            currentTarget: { ...e.currentTarget, name: props?.name || e.currentTarget?.name || '', value: e.currentTarget?.value },
        };
            ...
            <div className="relative">
                <input
                    ...
                    className={cn(
                        ...
                        showToggle && (size === 'sm' ? 'pe-8' : size === 'md' ? 'pe-10' : 'pe-12')
                    )}
                    {...props}
                />
                ...
                {showToggle && (
                    <button
                        ...
                        disabled={disabled}
                        className={cn(
                            isRTL ? 'absolute left-3 top-1/2 -translate-y-1/2' : 'absolute right-3 top-1/2 -translate-y-1/2',
            ...
            {showStrength && currentValue.length > 0 && (
                <div className="mt-2 space-y-1">
                    ...
                    <p className={cn('text-xs', strength.score <= 2 ? 'text-red-500' : strength.score <= 3 ? 'text-yellow-500' : 'text-green-500')}>
                        {t(`common.password_strength.${strength.labelKey}`) || strength.labelKey.replace('_', ' ')}
                    </p>
                </div>
            )}
        </InputWrapper>
